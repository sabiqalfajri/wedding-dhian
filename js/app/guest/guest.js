...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        if (name) {
            ...
            const template = `<small class="mt-0 mb-1 mx-0 p-0 fw-semibold" style="color: #2f2f2f;">${util.escapeHtml(guestName?.getAttribute('data-message'))}</small><p class="m-0 p-0" style="font-size: 1.25rem">${util.escapeHtml(name)}</p>`;
